...
/**
 * Provides the bundled Moonshine Tiny Streaming English model.
 *
 * The model bundle is packaged inside the APK at
 * assets/models/moonshine_stt_tiny_en.zip and extracted once into
 * app-private storage. No network access is performed.
 */
@Singleton
class MoonshineSttModelStore @Inject constructor(
    ...
    suspend fun ensureBundledModelAvailable(): File = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (!isComplete(tempRoot)) {
                throw IOException("Bundled Moonshine Tiny model is incomplete")
            }
            ...
            tempRoot.deleteRecursively()
            throw IOException(
                "Could not extract bundled Moonshine STT model: " + e.message,
                e
            )
        }
    }
    ...
    companion object {
        ...
        // Required by Moonshine Tiny Streaming. The spelling CNN assets are
        // intentionally excluded to keep the bundled app smaller.
